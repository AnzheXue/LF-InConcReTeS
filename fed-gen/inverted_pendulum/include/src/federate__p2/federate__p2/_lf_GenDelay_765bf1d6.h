...
#ifndef __LF_GENDELAY_765BF1D6_H // necessary for arduino-cli, which automatically includes headers that are not used
...
#endif
typedef struct _lf_gendelay_765bf1d6_self_t{
    self_base_t base; // This field is only to be used by the runtime, not the user.
    interval_t delay;
    int end[0]; // placeholder; MSVC does not compile empty structs
} _lf_gendelay_765bf1d6_self_t;
typedef struct {
    token_type_t type;
    lf_token_t* token;
    size_t length;
